...
class MonthlyStudentReportController extends Controller
{
    ...
    public function filterReports(Request $request)
    {
        ...
        $studentReports = $query->when($year, function ($query) use ($year) {
            ...
            ->offset($start)
            ->limit($length)
            ->get(['id', 'year', 'month', 'active_students_start', 'active_students_end', 'dropouts', 'dropout_percentage', 'new_students', 'new_students_percentage', 'turnaround', 'previous_month_difference', 'growth_rate', 'retention_rate', 'rotation']);
